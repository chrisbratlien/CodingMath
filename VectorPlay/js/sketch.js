...
function plotPolyVecs(polyVecs) {
  let colors = ["white", "teal", "magenta", "orange", "pink"];
...
function funGraph(ctx, axesOpts, func, color, thick) {
  ...
  var iMax = Math.round(vsub(rightCenter, leftCenter)[0] / dx);
  let iMin = 0;
  if (axes.doNegativeX) {
...
Tr.xyzw.rotateZ = function (theta) {
  //around Z axis
  let [cos, sin, negSin] = Tr.rotateMemo(theta);
  return [
    [cos, negSin, 0, 0],
...
Tr.xyzw.rotateX = function (theta) {
  // around X axis
  let [cos, sin, negSin] = Tr.rotateMemo(theta);
  let result = [
    [1, 0, 0, 0],
...
Tr.xyzw.rotateY = function (theta) {
  // around Y axis
  let [cos, sin, negSin] = Tr.rotateMemo(theta);
  return [
    [cos, 0, sin, 0],
...
window.onload = function () {
  ...
  setup = function () {
    ...
    let leftTop = vadd(center, [0, height / 2]);
    ...
    T = mmult(Tr.xy.scale([0.2, 2.5]), Tr.xy.rotate(theta));
  ...
  render = function () {
    da = 0.01;
    a += da;
    poly = mmult(Tr.xy.rotate(da), poly);
    ...
    poly3DWXY = [poly3DW[0][0], poly3DW[0][1]];
    ...
    plotPolyMatrix(mmult(Tr.xyzw.toXY, poly3DW));
    context.stroke();
    context.restore();
  ...
  campfire.emit("render-fn", render);
